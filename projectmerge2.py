...
# UFO rotation speed
ufo_rotation_speed = 0.2  # Adjust this value for slower or faster rotation
ufo_rotation_angle = 0.0

def draw_ufo(pos_x=0, pos_y=0, pos_z=0, scale=1.0, rotate_x=0, rotate_y=0, rotate_z=0):
    global ufo_rotation_angle
    
    # Increment the rotation angle at a slow pace
    ...
    glRotatef(ufo_rotation_angle, 0, 1, 0)  # Rotate UFO around Y-axis (you can change axes if needed)
...
def display():
    ...
    draw_ships()
    draw_obstacles()
    draw_ufo(pos_x=-150, pos_y=ufo_pos_y, pos_z=0, scale=0.4)
    glutSwapBuffers()
...
def key_pressed(key, x, y):
    ...
    if key == GLUT_KEY_UP:  # Move UFO up
        ufo_pos_y += 5
        ufo_pos_y = min(ufo_pos_y, WORLD_H / 2 - offset)

    elif key == GLUT_KEY_DOWN:  # Move UFO down
        ufo_pos_y -= 5
        ufo_pos_y = max(ufo_pos_y, -WORLD_H / 2 + offset)
...
def main():
    ...
    glutIdleFunc(idle)
    glutSpecialFunc(key_pressed)  # Register key events for special keys (up and down)
    glutMainLoop()
